...
conv1_full_RF.fit(convolved_image.reshape(len(convolved_image), -1), cifar_tl)

## Test (after ConvRF 1 and Full RF)
conv1_map_test = conv1.convolve_predict(cifar_testi)
mnist_test_preds = conv1_full_RF.predict(conv1_map_test.reshape(len(cifar_testi), -1))
