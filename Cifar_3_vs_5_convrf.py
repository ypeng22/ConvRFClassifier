...
for fraction_of_train_samples in fraction_of_train_samples_space:
   ...
   # accuracy vs num training samples (naive_rf)
conv_rf_predict_proba = list()
fraction_of_train_samples_space = np.geomspace(0.91, 1.0, num=10)
for fraction_of_train_samples in fraction_of_train_samples_space:
    conv_rf_pp = ConvRFClassifier_predict_proba.ConvRFClassifier(layers = 1, kernel_size = (10,), stride = (2,))
    best_accuracy = np.mean([run_rf(cifar_train_images, cifar_train_labels, cifar_test_images, cifar_test_labels, fraction_of_train_samples) for _ in range(1)])
    conv_rf_predict_proba.append(best_accuracy)
    print("Train Fraction:", str(fraction_of_train_samples))
    print("Accuracy:", str(best_accuracy))
    
# accuracy vs num training samples (one layer cnn)
conv_rf_predict = list()
fraction_of_train_samples_space = np.geomspace(0.01, 1.0, num=10)
for fraction_of_train_samples in fraction_of_train_samples_space:
    conv_rf_predict = ConvRFClassifier_predict.ConvRFClassifier(layers = 1, kernel_size = (10,), stride = (2,))
    best_accuracy = np.mean([run_rf(conv_rf_predict, cifar_train_images, cifar_train_labels, cifar_test_images, cifar_test_labels, fraction_of_train_samples) for _ in range(2)])
...
ax.plot(fraction_of_train_samples_space*100, conv_rf_apply, marker='X', markerfacecolor='red', markersize=10, color='green', linewidth=3, linestyle="--", label="Conv RF apply")
ax.plot(fraction_of_train_samples_space*100, conv_rf_predict, marker='X', markerfacecolor='red', markersize=10, color='green', linewidth=3, label="Conv RF predict")
